...
import { useRestoreWarehouse } from "../../../../Hooks/useWarehouses";
...
const RestoreConfirmation: React.FC<Props> = props => {
  ...
  const { mutateAsync, isLoading: restoreLoading } = useRestoreWarehouse();

  const handleRestore = async () => {
    mutateAsync({ warehouseId: row?.id }).then(() => {
      onClose();
    });
  };
  return (
      ...
      <DialogContent>
        <Stack direction={"row"} alignItems={"center"} justifyContent={"start"} gap={1}>
          <Typography fontSize={16} color={"black"} fontWeight={"bold"}>
            This will restore the Warehouse{" "}
          </Typography>
          <Typography
            color={"primary.main"}
            fontSize={16}
