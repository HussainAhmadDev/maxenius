...
const WarehouseDrawer: React.FC<WarehouseDrawerProps> = props => {
  const { onClose, open, row, onDelete, isTrash } = props;
  const navigate = useNavigate();
  const { data, isLoading: fetchLoading } = useWarehouseById(
    open && row?.id ? row?.id : undefined
  );
  const handleEdit = () => {
    if (row?.id) {
      navigate(`/admin/edit-warehouse/${row?.id}`);
    }
  };
  return (
      ...
      <Grid container spacing={fetchLoading ? 0 : 2}>
        ...
        <Grid item xs={6}>
          ...
          {fetchLoading ? (
            ...
            <Typography variant="body2" fontWeight={"bold"}>
              {!!data?.updated && data?.updated?.toLowerCase() === "none"
                ? "---"
