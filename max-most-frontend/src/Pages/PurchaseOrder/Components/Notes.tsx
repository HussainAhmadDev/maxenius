...
import { useNotes } from "../../../Hooks/usePurchaseOrder";
...
const Notes: React.FC<NotesProps> = ({ loading }) => {
  const [activeMode, setActiveMode] = useState<Mode>("public");
  const { orderId } = useParams();
  const { data: notesData } = useNotes(orderId || "");
  const [open, setOpen] = useState(false);
  const { palette } = useTheme();
  const notes = useMemo(() => {
    return {
      public: notesData?.results?.filter(note => note.type === "p") || [],
      private: notesData?.results?.filter(note => note.type === "v") || []
    };
  }, [notesData]);
      ...
      <Card>
        <Stack p={2} direction={"row"} gap={2} justifyContent={"start"} flexWrap={"wrap"}>
            ...
            : modes.map((item, key) => (
                ...
                >
                  <Typography
                    ...
                    fontSize={14}
                    noWrap
                    id={item === "private" ? "cy__OrderPrivateNotes" : undefined} // Conditional id
                  >
                    Order {item} Notes{" "}
                  ...
                  {activeMode === item && (
                    <Button
                      ...
                      id={`cy__Add${activeMode?.charAt(0)?.toUpperCase() + activeMode?.slice(1)}Note`}
                    >
                      Add Note
                    </Button>
                  )}
          ...
          <SeeDocumentation
            ...
            title={"See Documentation of PurchaseOrder Notes"}
          />
        </Stack>
        <Divider />
        <CardContent>
                ...
                {[...(notes?.[activeMode] || [])].map((note, key, arr) => {
                  ...
                  return (
                    <React.Fragment key={key}>
                      <SeeDocumentation
                        fileName={"useNotes"}
                        title={"See Notes Listing Documentation"}
                      />
        ...
        <AddNote mode={activeMode} onClose={handleClose} open={open} id={orderId} />
      </Card>
    </>
  );
};
