...
const FilterTable: React.FC<Props> = ({ handleSelectCustomer }) => {
  ...
  const [websiteOrderID, setWebsiteOrderID] = React.useState<string>();
  ...
  const { handleSave, handleModalOpen, handleModalClose, modalOpen } = useModal({
    onSave: () => {
      if (currentCustomer && websiteOrderID) {
        createOrder({
          company_id: currentCustomer?.id,
          brand_id: currentCustomer?.brand_id || "",
          contact_id: currentCustomer?.billing_contact?.id || "",
          website_id: websiteOrderID
        });
      } else {
        !websiteOrderID && toast.error("Please select the website");
      }
    }
  });
  ...
  return (
    <div>
      <PopupNotes
        ...
        setWebsiteOrderFun={id => setWebsiteOrderID(id)}
        openModal={modalOpen}
