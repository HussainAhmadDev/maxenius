...
export const useLazyPatientHistory = () => {
  return async (
    ...
    // const generalParams: Record<string, string> = {
    //   is_trash: "1" // You can customize this as needed
    // };
...
//Get all prescription WebSites
interface PrecriptionWebsite {
  count: number;
  page: number;
  pages: number;
  results: Results[];
...
export const useWebsitesPrescription = () => {
  const queryClient = useQueryClient();
  const localBrand = localStorage.getItem("brand");
  const { activeBrand } = useBrand();
  const generalParams: Record<string, string> = {
    brand_id: (localBrand as string) || activeBrand
  };

  return useQuery<PrecriptionWebsite, Error>(
    ["prescriptionwebsites"],
    async () => {
      const response = await fetch(
    ...
    },
    {
      ...
      onSuccess: () => {
        // Cache the data and invalidate previous queries
