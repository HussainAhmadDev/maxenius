...
//      * @returns {JiriPoint | null}
//      */
//     JiriPoint: (point, point2) =>
//       point.x === point2.x && point.y === point2.y ? point : null,
//     /**
//      *
//      * @param {JiriPoint} point
//      * @param {Ray} ray
...
//      */
//     Circle: (point, circle) =>
//       point.distance(circle.center) == jiri_round(circle.radius, 'coordinate')
//         ? point
//         : null,
//     /**
//      *
...
//       polygon.edges.map((v) => v.getIntersect(point)).filter((v) => v !== null).length > 0
//         ? point
//         : null,
//     /**
//      *
...
//       const a1 = line.getLinePolynom().getLinearCoefficient();
//       const b1 = line.getLinePolynom().getAbsoluteCoefficient();
//       const a2 = line2.getLinePolynom().getLinearCoefficient();
//       const b2 = line2.getLinePolynom().getAbsoluteCoefficient();
//       if (a1 === a2) {
//         if (b1 === b2) {
//           return line;
//         }
...
//       const l = segment.getLine();
//       const l2 = segment2.getLine();
//       const i = l.getIntersect(l2);
//       if (i === null) return null;
//       if (i instanceof JiriPoint)
//         return i.intersects(segment) && i.intersects(segment2) ? i : null;
...
//             return segment;
//           } else {
//             return [segment, segment2].find(
//               (v) => v.a.distance(t[3]) === 0 || v.b.distance(t[3]) === 0,
//             );
//           }
//         }
//         if (t[1].distance(t[2]) === 0) return t[1];
//         if (
//           t[1].intersects(segment) &&
//           t[2].intersects(segment) &&
//           t[1].intersects(segment2) &&
//           t[2].intersects(segment2)
//         ) {
...
//      */
//     Ray: (ray, ray2) => {
//       const [l, l2] = [ray, ray2].map((v) => v.getLine());
//       const i = l.getIntersect(l2);
//       if (i === null) return null;
//       else {
//         if (i instanceof JiriLine) {
//           if (ray.a.distance(ray2.a) === 0 && ray.b.distance(ray2.b) === 0) return ray;
//           if (ray.intersects(ray2.a) && ray2.intersects(ray.a))
//             return new Segment(ray.a, ray2.a);
...
//       if (circle.center.distance(circle2.center) === 0)
//         return jiri_round(circle.r) === jiri_round(circle2.r) ? circle : null;
//       if (circle.center.distance(circle2.center) > jiri_round(circle.r + circle2.r))
//         return null;
//       const l = new Segment(circle.center, circle2.center);
//       const rRatio = circle.r / (circle.r + circle2.r);
//       let dx = circle.center.x - circle2.center.x;
//       if (circle.center.distance(circle2.center) === jiri_round(circle.r + circle2.r)) {
//         if (l.a.x == l.b.x) {
//           const ya =
//             max(l.a.y, l.b.y) -
//             [circle, circle2].find((c) => c.center.y == max(l.a.y, l.b.y)).r;
//           return new JiriPoint(l.a.x, ya);
//         }
//         const pdx = dx * rRatio;
//         const x = circle.center.x + pdx;
//         const y = l.y(x);
//         return new JiriPoint(x, y);
//       }
//       if (circle.center.x === circle2.center.x) {
//         var angle1cos =
//           (l.length() ** 2 + circle.r ** 2 - circle2.r ** 2) /
//           (2 * l.length() + circle2.r);
...
//         var c = this.p5.sin(acos(angle1cos)) * circle.r;
//         return [new JiriPoint(y, x - c), new JiriPoint(y, x + c)];
//       }
//       var angle1cos =
//         (l.length() ** 2 + circle.r ** 2 - circle2.r ** 2) / (2 * l.length() * circle2.r);
//       var k = angle1cos * circle.r;
//       var kRatio = k / l.length();
//       var pdx = dx * kRatio;
//       var x = circle.center.x - pdx;
...
//       var c = this.p5.sin(acos(angle1cos)) * circle.r;
//       var p = new JiriPoint(x, y);
//       const p2 = new JiriPoint(p.x, circle.center.y);
//       if (p.distance(p2) === 0)
//         return [new JiriPoint(p.x, p.y + c), new JiriPoint(p.x, p.y - c)];
//       const t = new Triangle(circle.center, p2, p);
//       const angle2 = t.getGamma();
//       const angle3 = this.p5.PI / 2 - angle2;
//       const dy = -cos(angle3) * c;
//       dx = sin(angle3) * c;
...
//         const l = line.getPerpendicular(circle.center);
//         return l.getIntersect(line);
//       }
//       const h = line.getPerpendicular(circle.center);
//       const d = line.distance(circle.center);
//       const l = pow(circle.r ** 2 - d * d, 1 / 2);
//       if (line.intersects(circle.center)) {
//         if (line.a.x === line.b.x) {
//           return [
//             new JiriPoint(circle.center.x, circle.center.y - circle.r),
//             new JiriPoint(circle.center.x, circle.center.y + circle.r),
//           ];
//         }
//         const s = new Segment(line.a, line.b);
//         const angle = acos((s.b.x - s.a.x) / s.length());
//         const dx = cos(angle) * l;
//         const dy = sin(angle) * l;
//         let p = new JiriPoint(circle.center.x + dx, circle.center.y + dy);
//         if (!p.intersects(line))
//           p = new JiriPoint(circle.center.x + dx, circle.center.y - dy);
//         return [p, p.reflectAbout(circle.center)];
//       }
//       const c = new Circle(line.getIntersect(h), l);
...
//       if (is instanceof Array) {
//         const a = is.filter((i) => i.intersects(segment));
//         if (a.length === 0) return null;
//         if (a.length === 1) return a[1];
//         return a;
//       }
...
//     Ray: (polygon, ray) => {
//       const l = ray.getLine();
//       const i = l.getIntersect(polygon);
//       if (i === null) return null;
//       if (i instanceof Segment || i instanceof JiriPoint) return i.getIntersect(ray);
//       if (i instanceof Array)
//         return i.map((v) => v.getIntersect(ray)).filter((v) => v !== null);
//     },
//     /**
//      *
//      * @param {Polygon} polygon
...
//       if (i instanceof Array)
//         return i.map((v) => v.getIntersect(segment)).filter((v) => v !== null);
//     },
//     /**
//      *
//      * @param {Polygon} polygon
...
//       const ps = polygon.vertices
//         .map((v) => v.getIntersect(circle))
//         .filter((v) => v !== null)
//         .flat(3);
//       //Removes duplicates
...
//         for (var a of c) {
//           if (a instanceof JiriPoint) continue;
//           const g = a.getIntersect(v);
//           if (g === null) continue;
//           if (g instanceof Segment) c.push(v.join(a));
//           if (g instanceof JiriPoint) {
//             if ([v.a, v.b, a.a, a.b].filter((j) => j.intersects(v)).length === 4)
//               c.push(v.join(a));
//           }
//         }
//       }
...
//      * @returns {Array<Segment | JiriPoint> | Segment | JiriPoint | null}
//      */
//     Triangle: (triangle, trinagle2) =>
//       intersectFunctions.Polygon.Polygon(triangle, trinagle2),
//     /**
//      *
//      * @param {Triangle} triangle
//      * @param {Circle} circle
...
//     } catch (err) {
//       throw new Error(
//         'Intersect for these two objects has not been yet defined. If you belive this is a mistake, please report this on the offical GitHub page. Objects: ' +
//           this +
//           ' and ' +
//           object,
//       );
...
//       if (!(p instanceof JiriPoint))
//         throw new TypeError('All vericies of a polygon must be Points.');
//     }
//     super();
//     /**
...
//     const angleIncrement = (PI * 2) / vertices;
//     const vects = [];
...
//     if (!(point1 instanceof JiriPoint))
//       throw new TypeError('The point1 argument must be a type of Point.');
//     if (!(point2 instanceof JiriPoint))
//       throw new TypeError('The point2 argument must be a type of Point.');
//     if (point1.distance(point2) === 0)
//       throw new Error('Need two different points to construct a line.');
//     super();
//     /**
//      * @description The first point of the line
//      * @type {JiriPoint}
//      */
//     this.a = [point1, point2].find((p) => p.x === min(point1.x, point2.x));
//     /**
...
//    */
//   y(x) {
//     const b = (this.b.y * this.a.x - this.a.y * this.b.x) / (this.a.x - this.b.x);
//     const a = (this.b.y - this.a.y) / (this.b.x - this.a.x);
...
//       const pline = this.getParallel(point);
//       return pline.getPerpendicular(point);
//     }
//     const angle = atan((this.a.x - this.b.x) / (this.a.y - this.b.y));
//     const x = -cos(angle);
//     const y = sin(angle);
...
//     const l = new JiriLine(point, new JiriPoint(point.x, point.y - 1));
//     const p = l.getIntersect(this);
//     if (p === null) return l;
//     const p2 = new JiriPoint(p.x - 1, this.y(p.x - 1));
...
//   constructor(x, y) {
//     if (typeof x !== 'number')
//       throw new TypeError('The x coordinate of a point must be a number.');
//     if (typeof y !== 'number')
//       throw new TypeError('The y coordinate of a point must be a number.');
//     super();
//     /**
//      * @type {Number}
...
//       return jiri_round(
//         pow(pow(this.x - object.x, 2) + pow(this.y - object.y, 2), 1 / 2),
//         'coordinate',
//       );
//     if (object instanceof JiriLine) {
//       const l = object.getPerpendicular(this);
//       const p = l.getIntersect(object);
...
//     }
//     if (object instanceof JiriLine) {
//       const perpendicular = object.getPerpendicular(this);
//       const i = perpendicular.getIntersect(object);
...
//     const a = string.split('[').pop().split(']')[0].split(', ');
...
//         throw new TypeError('All coefficients of a polynom must be numbers.');
//     }
//     /**
//      * @type {Number}
...
//    */
//   valueAt(x) {
//     return this.coefficients.map((v, i) => v * pow(x, i)).reduce((a, b) => a + b);
//   }
//   /**
...
//     if (!(endPoint instanceof JiriPoint))
//       throw new TypeError('The endPoint argument must be a type of Point.');
//     if (!(point2 instanceof JiriPoint))
//       throw new TypeError('The point2 argument must be a type of Point.');
//     super();
//     /**
...
//       throw new TypeError('The point2 argument must be a type of Point.');
//     if (point1.distance(point2) === 0)
//       throw new Error('Need two different points to construct a segment.');
//     super();
//     /**
...
//      * @type {JiriPoint}
//      */
//     this.b = [point1, point2].find((p) => p.x === max(point1.x, point2.x));
//   }
//   /**
...
//       throw new TypeError('The segemnt argument must be a Segment.');
//     const i = this.getIntersect(segment);
//     if (i === null) throw new Error('Cannot join segments that do not intersect.');
//     if (i instanceof JiriPoint) {
//       if (!(this.getLine().getIntersect(this) instanceof Segment))
//         throw new Error('Cannot join segments that do not lay on one line.');
...
//    * @returns {Number}
//    */
//   length(p5: P5CanvasInstance) {
//     return p5.PI * 2 * this.r;
//   }
//   /**
...
//    */
//   constructor(a, b, c) {
//     if (!(a instanceof JiriPoint))
//       throw new TypeError('The a argument must be a type of Point.');
//     if (!(b instanceof JiriPoint))
//       throw new TypeError('The b argument must be a type of Point.');
//     if (!(c instanceof JiriPoint))
//       throw new TypeError('The c argument must be a type of Point.');
//     if (new JiriLine(a, b).intersects(c))
//       throw new Error(
//         'Cannot construct a triangle from three points that lay on the same line.',
//       );
//     if (a.distance(b) === 0 || a.distance(c) === 0 || c.distance(b) === 0)
//       throw new Error('Points on a triangle cannot match.');
//     super([a, b, c]);
//     /**
...
//     if (object instanceof Polygon || object instanceof Triangle)
//       return this.drawPolygon(object);
//     throw new TypeError(
//       'Cannot draw this object. If you believe this is not correct, please report this on the offical GitHub page. Object:' +
...
//    */
//   drawPoint(point) {
//     if (!(point instanceof JiriPoint))
//       throw new TypeError('The point argument must be a type of Point.');
//     const d = this.canvas.context.lineWidth;
//     this.canvas.context.beginPath();
//     this.canvas.context.arc(point.x, point.y, d * 2, 0, this.p5.PI * 2);
...
//   drawLine(line) {
//     if (!(line instanceof JiriLine))
//       throw new TypeError('The line argument must be a type of Line.');
//     if (line.a.x === line.b.x) {
...
//       throw new TypeError('The ray argument must be a type of Ray.');
//     if (ray.a.x === ray.b.x) {
...
//   drawSegment(segment) {
//     if (!(segment instanceof Segment))
//       throw new TypeError('The segment argument must be a type of Segment.');
...
//       throw new TypeError('The circle argument must be a type of Circle.');
...
//       throw new TypeError('The polygon argument must be a type of polygon.');
//     this.canvas.context.beginPath();
...
//     if (values.length == 1)
//       console.warn(
//         'You are trying to use a vector with just one value. Consider using the native number class for better performance.',
//       );
//     this.push(...values);
//   }
//   /**
...
//       throw new Error("Can not multiply vectors, that don't have the same length.");
//     var v = [];
...
//     }
//     if (vector.length != this.length)
//       throw new Error("Can not divide vectors, that don't have the same length.");
//     var v = [];
//     for (let i = 0; i < vector.length; i++) {
//       v.push(this[i] / vector[i]);
//     }
...
//     }
//     if (vector.length != this.length)
//       throw new Error("Can not add vectors, that don't have the same length.");
//     var v = [];
//     for (let i = 0; i < vector.length; i++) {
//       v.push(this[i] + vector[i]);
//     }
...
//     }
//     if (vector.length != this.length)
//       throw new Error("Can not minus vectors, that don't have the same length.");
//     var v = [];
//     for (let i = 0; i < vector.length; i++) {
//       v.push(this[i] - vector[i]);
//     }
...
//     const v = this.setMagnitude(limit);
...
//   angle() {
//     if (this.length != 2)
//       throw new Error('This method is only defined for a two dimensional vector.');
