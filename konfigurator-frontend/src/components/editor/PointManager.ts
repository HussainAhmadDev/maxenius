...
import { showModel } from './constants';
import { Angle, Grid, Line, Point } from './elements';
import { FloorPlanManager } from './floorPlanManager';
import { isInFrame, mousePos } from './others';

export class PointManager {
  parent: FloorPlanManager | undefined;
  lastSent: any;
  dragging: any;
  additionalLines: any;
  closingLine: any;
  ...
  grid: Grid;
  points: Point[];
  doUpdateCall: boolean;
  convertToUnit: boolean;
  remove180DegCorners: boolean;
  hoveringObject: null | Line | Point;
  additionalPoints: any;
  finished: boolean;
  changingData: any;
  relativeAngles: number[];
  getRelativeAngle: (id: any) => number;
  closingAngle: number;
  allowHover: boolean;
  changeLineLengthConservative: boolean;
  trashPM: PointManager | null;
  updateCall: () => void;
  startMousePos: { x: number; y: number } | undefined;
  startObj: null | Point | Line | undefined;
  ...
  constructor(points: Array<any> = []) {
    ...
    // ANGLES
    this.relativeAngles = [];
    this.getRelativeAngle = function (id) {
      const { canvasInstance } = useEditorStore.getState();
      return (this.relativeAngles[id] * 180) / canvasInstance.PI;
    };
    ...
    this.changingData = {
      ...
      itemPos: {
        // the position of the item (the input gets moved to thes position)
        x: -100,
        y: -100,
      },
  ...
  updateTrashPM(newTrashPM: PointManager) {
    this.trashPM = newTrashPM;
  }
  ...
  update() {
    ...
    for (let i = 1; i < this.points.length; i++) {
      if (this.lines[i - 1] !== undefined) {
        const lastCol = this.lines[i - 1].c;
    ...
    if (this.closed) {
      ...
      this.closingAngle =
        canvasInstance.PI - this.closingLine.angle + this.lines[0].angle;
      this.closingAngle =
        2 * canvasInstance.PI -
        ((this.closingAngle + 2 * canvasInstance.PI) % (2 * canvasInstance.PI));
    }

    // Recalculate all lengths of additionalLines
    for (const al of this.additionalLines) {
      al.update();
      al.isOuter = false;
    }
    ...
    for (let p = 0; p < this.points.length; p++) {
      if (this.points[p]?.angle !== null) {
        if (
          canvasInstance.round(this.points[p].angle!.relative, 5) ==
            canvasInstance.round(canvasInstance.PI, 5) &&
          !this.points[p].connectedToAddLine &&
          this.remove180DegCorners
        ) {
        ...
        if (this.points[p]?.id) this.points[p].id = p;
      }
    }

    if (this.doUpdateCall) {
      this.updateCall();
    }
  }
   ...
   */
  draw(includeDescriptors: boolean = true) {
    ...
    if (this.points.length != 0) {
      ...
      for (const a of this.additionalLines) {
        if (a.mouseOver()) {
          this.hoveringObject = a;
        }
        a.draw(true);
      }
      for (const a of this.additionalPoints) {
        if (a.mouseOver()) {
          this.hoveringObject = a;
        }
        a.draw(true);
      ...
      // If there are no points -> start drawing
      this.finished = false;
    }

    // If there is data being changed
    if (this.changingData.changing) {
      // const off = document.getElementById('canvas-container')?.getBoundingClientRect();
    ...
    if (!this.finished) {
      ...
      if (this.points.length > 0) {
        ...
        const l = new Line(
          this,
          new Point(this, lastP.x, lastP.y),
          new Point(this, mP.x, mP.y),
        );
        if (this.lines[this.lines.length - 1]) {
          let a = canvasInstance.abs(
            canvasInstance.PI -
              l.angle +
              this.lines[this.lines.length - 1].angle +
              canvasInstance.PI,
          );

          a = canvasInstance.PI - ((a + 2 * canvasInstance.PI) % (2 * canvasInstance.PI));

          if (a > canvasInstance.PI) {
            a = 2 * canvasInstance.PI - a;
          }

          let ang;
          if (a > 0) {
            a = canvasInstance.abs(a);

            ang = new Angle(
              this,
              lastP.x,
              lastP.y,
              this.lines[this.lines.length - 1].angle - canvasInstance.PI,
              l.angle,
              -1,
            );
          } else {
          ...
          ang.draw(true, false);
          ang.descriptors[0].text =
            canvasInstance
              .round((canvasInstance.abs(a) * 180) / canvasInstance.PI, 2)
              .toString() + '°';
          ang.drawDescriptors();
        }
    ...
    // console.log('🚀 ~ PointManager ~ draw ~ PointManager drawing working:');
  }

  /**
   * The recursive function for drawing the whole barebones structure.
   * @param {Point[]} arrP A copy of the points array
   * @param {Line[]} arrL A copy of the lines array
   * @param {Boolean} isFirst A Boolean which tells us, if this is depth 0 of the recursion (true by default)
   */

  drawRecursive(arrP: Point[], arrL: Line[], isFirst: boolean = true) {
    const { canvasInstance } = useEditorStore.getState();
    // If the point is first
    if (isFirst) {
      canvasInstance.push();
      //Translate
      canvasInstance.translate(this.points[0].x, this.points[0].y);
    }
    ...
    if (arrL.length == 0) {
      ...
      // End recursion
      if (this.closed) {
        // Draw the closing line if the pointManager is closed
    ...
    canvasInstance.rotate(arrL[0].angle - canvasInstance.PI / 2);

    // Draw the first line at false position
    arrL[0].draw(false);

    // Translate along the length of the line
    canvasInstance.translate(0, arrL[0].length);
    // Revert the rotation
    canvasInstance.rotate(-arrL[0].angle + canvasInstance.PI / 2);
  ...
  keyPressed = () => {
    ...
    if (!this.changingData.changing && canvasInstance.keyCode == 13) {
  ...
  setChanges() {
    ...
    let num = parseFloat(this.changingData.input.children[1].value);
    this.changingData.input.children[1].value = '';

    // Change the appropriate item from the right array to the value
    if (this.changingData.object.type == new Point(this).type) {
      // Change the angle, if the item was an Angle
      this.changeRelativeAngleOnScale(
        (canvasInstance.constrain(num, 0, 180) * canvasInstance.PI) / 180,
        this.changingData.itemID,
      );
    } else if (this.changingData.object.type == new Line(this).type) {
      if (this.convertToUnit) num = this.grid.toPixels(num);

      if (this.changeLineLengthConservative) {
        this.changeLengthConservative(
          num / 1000,
          this.changingData.object,
          this.changingData.object.points[0],
        );
      } else {
  ...
  mousePressed = (event: { which: number }) => {
    const { canvasInstance } = useEditorStore.getState();
    ...
    } else if (this.finished && !showModel) {
      // Finished and model is not showing
      // Hovering object cannot be null
      if (this.hoveringObject !== null) {
        if (event.which == 1) {
          //LEFT CLICK
          // Start the click
          this.changingData.clickFrame = canvasInstance.frameCount;
          ...
          // RIGHT CLICK
          if (this.hoveringObject.type == new Point(this).type) {
            // The hovering object is a point
            this.removePoint(this.hoveringObject as Point);
          } else if (this.hoveringObject.type == new Line(this).type) {
            // The hovering object is a line
            this.removeLine(this.hoveringObject as Line);
          }
        }
      }
    }
  ...
  mouseReleased = () => {
    ...
    if (
      this.dragging &&
      isInFrame(mousePos().x, mousePos().y) &&
      mousePos().x !== this.startMousePos?.x &&
      mousePos().y !== this.startMousePos?.y &&
      this.startObj !== null
    ) {
      this.dragging = false;
      this.updateCall();
    ...
    this.changingData.changing =
      canvasInstance.frameCount - this.changingData.clickFrame <=
      this.changingData.maxFrameDuration;
    if (this.changingData.changing) {
      const d: HTMLElement | null = document.getElementById('ProfileChangeDiv');

      if (d) d.innerHTML = '';

      document.getElementById('LineChangeDiv')?.appendChild(this.changingData.input);
      this.changingData.input.style.display = '';
      this.changingData.input.children[0].children[0].innerText =
        this.changingData.object.type == 'Point' ? 'Winkel (°)' : 'Länge (mm)';
      this.changingData.input.children[1].step =
        this.changingData.object.type == 'Point' ? '1' : '250';

      if (d) {
        d.innerHTML +=
          "<button class='btn btn-success' style ='margin-right: 10px;' onclick = 'securityPlan.mainGeometry.setChanges()'> Einstellen </button>";
        d.innerHTML +=
          "<button class='btn btn-danger' style ='margin-right: 10px;' onclick = 'securityPlan.mainGeometry.cancelChanges()'> Abbrechen </button>";
      }
      if (this.changingData.object.type == 'Point') {
        //print(round(this.changingData.object.angle.relative * 180 / PI, 3))
        this.changingData.input.children[1].value = canvasInstance.round(
          (this.changingData.object.angle.relative * 180) / canvasInstance.PI,
          3,
        );
      } else {
        //print(round(this.grid.toUnit(this.changingData.object.length) * 1000, 2))
        this.changingData.input.children[1].value = canvasInstance.round(
          this.grid.toUnit(this.changingData.object.length) * 1000,
          2,
        );
      }
    }
    ...
    for (const p of this.additionalPoints) {
      p.lock(false);
    }
    // unlock all points
    for (const l of this.lines) {
    ...
    for (const l of this.additionalLines) {
      l.lock(false);
    }
  };
  ...
  mouseDragged = () => {
    ...
    if (lockedPoints.length == 1) {
    ...
    } else if (lockedPoints.length == 2) {
      // Two points are locked -> a line is locked
      // Find the line which is dragged
      let l = this.lines[this.points.indexOf(lockedPoints[0])];

      if (
        this.points.indexOf(lockedPoints[0]) == -1 ||
        this.points.indexOf(lockedPoints[1]) == -1 ||
        lockedPoints[1].id - lockedPoints[0].id != 1
      ) {
        for (const lin of this.additionalLines) {
      ...
      if (
        this.points.indexOf(lockedPoints[0]) == 0 &&
        this.points.indexOf(lockedPoints[1]) == this.points.length - 1 &&
        this.closed
      ) {
        l = this.closingLine;
  ...
  calculateRelativeAngles() {
    ...
    if (this.lines.length !== 0) {
      ...
      for (let i = 1; i < this.lines.length; i++) {
        ...
        let ang: number =
          canvasInstance.PI - this.lines[i - 1].angle + this.lines[i].angle;

        // Clean up the angle and map it, so that it fits between 0 - 2*PI
        ang =
          2 * canvasInstance.PI -
          ((ang + 2 * canvasInstance.PI) % (2 * canvasInstance.PI));

        // Add the relative angle
        this.relativeAngles.push(ang);

        // Clean up all angles, so that they are not over 2*PI
        this.lines[i].angle %= 2 * canvasInstance.PI;
        this.relativeAngles[i] %= 2 * canvasInstance.PI;
      }
      ...
      for (let i = 1; i < this.points.length - 1; i++) {
        ...
        if (this.relativeAngles[i] > canvasInstance.PI) {
          // Left side
          this.points[i].setAngle(
            new Angle(
              this,
              this.points[i].x,
              this.points[i].y,
              this.lines[i].angle + this.relativeAngles[i],
              this.lines[i].angle,
              i - 1,
            ),
          );
  ...
  recalculatePointPositions() {
    ...
    let totalX = this.points[0].x;
    let totalY = this.points[0].y;
   ...
   */
  changeAbsoluteAngle(newA: number, id: number, shouldUpdate: boolean = true) {
   ...
   * @param {Number} id The index of the line
   */
  changeRelativeAngleOnScale(newA: number, id: number) {
    const { canvasInstance } = useEditorStore.getState();
    // Make sure that the index relates to a changeable angle
    if (this.checkAngleChangeIndex(id)) {
      //If the relative angle is larger than PI, the new angle has to be interpreted as 2*PI-newA
      if (this.relativeAngles[id] > canvasInstance.PI) {
        // Change the angle with interpretation
        this.changeRelativeAngle(2 * canvasInstance.PI - newA, id);
      } else {
   ...
   */
  checkAngleChangeIndex(id: number) {
   ...
   */
  addPoint(x: number, y: number, disableUpdate = false, stopClosing = false) {
    const { canvasInstance } = useEditorStore.getState();
    if (!stopClosing) {
      // If the position is of the new point is very close to the first point
      if (this.points.length > 0) {
        if (canvasInstance.dist(this.points[0].x, this.points[0].y, x, y) < 3) {
    ...
    const newP = new Point(this, x, y, this.points.length);
  ...
  removePoint(obj: Point) {
    const { canvasInstance } = useEditorStore.getState();
    let id = this.points.indexOf(obj);

    if (id != -1) {
      for (let al = 0; al < this.additionalLines.length; al++) {
        if (this.additionalLines[al].points.includes(obj)) {
          let ind = this.additionalLines[al].points.indexOf(obj);
          ind = ind == 0 ? 1 : 0;

          if (this.additionalLines[al].points[ind].angle != null) {
            if (
              this.additionalLines[al].points[ind].angle.relative == canvasInstance.PI
            ) {
              this.points.splice(
                this.points.indexOf(this.additionalLines[al].points[ind]),
                1,
              );
            }
          }
   ...
   */
  changeLength(newL: number, obj: Line) {
  ...
  /**
   * A function to change the scale of the lines.
   *
   ...
   */
  changeLengthConservative(newL: number, obj: Line, statP: Point) {
    const { canvasInstance } = useEditorStore.getState();
    for (const l of this.lines.concat(this.additionalLines)) {
      if (
        obj.points[0].x == l.points[0].x &&
    ...
    if (
      canvasInstance.round(statP.x, 5) == canvasInstance.round(obj.points[0].x, 5) &&
      canvasInstance.round(statP.y, 5) == canvasInstance.round(obj.points[0].y, 5)
    ) {
      obj.points[1].x = obj.points[0].x + canvasInstance.cos(obj.angle) * newL;
      obj.points[1].y = obj.points[0].y + canvasInstance.sin(obj.angle) * newL;
    } else if (
      canvasInstance.round(statP.x, 5) == canvasInstance.round(obj.points[1].x, 5) &&
      canvasInstance.round(statP.y, 5) == canvasInstance.round(obj.points[1].y, 5)
    ) {
      obj.points[0].x = obj.points[1].x - canvasInstance.cos(obj.angle) * newL;
      obj.points[0].y = obj.points[1].y - canvasInstance.sin(obj.angle) * newL;
    }
   ...
   */
  addLine(angle: number, length: number, right: boolean) {
    const { canvasInstance } = useEditorStore.getState();
    // If the angle is from the left, interpret the angle as 2*PI-angle
    if (!right) {
      angle = 2 * canvasInstance.PI - angle;
    }
    ...
    if (lastPoint == undefined) {
      ...
      this.addPoint(
        lastPoint.x + Math.cos(angle) * length,
        lastPoint.y + Math.sin(angle) * length,
      );
  ...
  /**
   * Function which removes a line at a specified index. The lengths of the other lines shuffle down, bu the relative angles are kept.
   * @param {Line} obj Reference to the object that is to be deleted.
   */
  removeLine(obj: Line) {
    const { canvasInstance } = useEditorStore.getState();
    let id = this.lines.indexOf(obj);

    //Trying to remove the closing line or additional line
    if (id == -1) {
      id = this.additionalLines.indexOf(obj);
      if (id != -1) {
        // It is an additional line
        const pnts = obj.points;
        ...
        for (const p of pnts) {
          ...
          if (this.additionalPoints.includes(p)) {
            ...
            if (p.angle?.relative == canvasInstance.PI) {
              shouldRemove = true;
            }
          }
    ...
    // We have to have at least one line to remove
      ...
      for (let i = id; i < this.lines.length - 1; i++) {
  ...
  /**
   * Adds an additional line to the current set of points, with the option to disable update.
   *
   * @param {Point} p1 - description of parameter
   * @param {Point} p2 - description of parameter
   * @param {boolean} disableUpdate - description of parameter
   */
  addAdditionalLine = (p1: Point, p2: Point, disableUpdate: boolean = false) => {
    const { canvasInstance } = useEditorStore.getState();
    let point1;
    let point2;

    if (
      p1?.x == this.points[this.points.length - 1]?.x &&
      p1?.y == this.points[this.points.length - 1]?.y
    ) {
      this.addPoint(p2.x, p2.y, disableUpdate);
      this.update();
      return;
    } else if (
      p2.x == this.points[this.points.length - 1]?.x &&
      p2.y == this.points[this.points.length - 1]?.y
    ) {
    ...
    if (p1.x == this.points[0]?.x && p1.y == this.points[0]?.y) {
      this.points.unshift(new Point(this, p2.x, p2.y, -1));
      this.update();
      return;
    } else if (p2.x == this.points[0].x && p2.y == this.points[0].y) {
      this.points.unshift(new Point(this, p1.x, p1.y, -1));
      this.update();
      return;
    }

    const allPoints = this.points.concat(this.additionalPoints);
    for (const p of allPoints) {
      if (
        canvasInstance.round(p.x, 5) == canvasInstance.round(p1.x, 5) &&
        canvasInstance.round(p.y, 5) == canvasInstance.round(p1.y, 5)
      ) {
        p.connectedToAddLine = true;
        point1 = p;
      } else if (
        canvasInstance.round(p.x, 5) == canvasInstance.round(p2.x, 5) &&
        canvasInstance.round(p.y, 5) == canvasInstance.round(p2.y, 5)
      ) {
        p.connectedToAddLine = true;
        point2 = p;
      }
    ...
    /**
     * Calculate whether a point is on a line segment defined by two endpoints.
     *
     * @param {object} l1 - the first endpoint of the line segment
     * @param {object} l2 - the second endpoint of the line segment
     * @param {object} p - the point to be checked
    ...
    const onLine = (
      ...
      const d12 = canvasInstance.dist(l1.x, l1.y, l2.x, l2.y);
      const d13 = canvasInstance.dist(p.x, p.y, l2.x, l2.y);
      const d23 = canvasInstance.dist(l1.x, l1.y, p.x, p.y);
    ...
    for (const l of this.lines) {
      ...
      if (point1 == undefined) {
        if (onLine(l.points[0], l.points[1], p1)) {
          point1 = new Point(this, p1.x, p1.y, -1);
      ...
      if (point2 == undefined) {
        if (onLine(l.points[0], l.points[1], p2)) {
          point2 = new Point(this, p2.x, p2.y, -1);
    ...
    for (const l of this.additionalLines) {
      ...
      if (point1 == undefined) {
        if (onLine(l.points[0], l.points[1], p1)) {
          ...
          const l1 = new Line(this, l.points[0], point1);
          l1.c = 'purple';
          this.additionalLines.push(l1);

          const l2 = new Line(this, l.points[1], point1);
          l2.c = 'purple';
      ...
      if (point2 == undefined) {
        if (onLine(l.points[0], l.points[1], p2)) {
          ...
          const l1 = new Line(this, l.points[0], point2);
          l1.c = 'purple';
          this.additionalLines.push(l1);

          const l2 = new Line(this, l.points[1], point2);
          l2.c = 'purple';
          this.additionalLines.push(l2);
    ...
    if (point1 != undefined && point2 != undefined) {
      const ad = new Line(this, point1, point2);
      ad.c = 'purple';
